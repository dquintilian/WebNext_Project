...
async function fetchArticleById(entryId: string): Promise<BlogPost | null> {
  try {
    ...
    const locale = entry.sys.locale ?? "default-locale";
    // Return a BlogPost structure
    return {
      sys: {
        ...entry.sys,
        locale,
      }, // Include sys metadata
      fields: {
        articleTitle: title,
        articleBody: body,
      },
...
export default async function BlogPostPage({
  ...
  // Destructure title and body from post.fields
  const { articleTitle: title, articleBody: body } = post.fields;
