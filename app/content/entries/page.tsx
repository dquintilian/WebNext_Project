...
import { fetchEntries } from "../../../lib/utils/articleList"; // Import a function to fetch a list of article entries, adjust the path as needed
...
export default async function HomePage() {
  ...
  const entries: EntryCollection<EntrySkeletonType> | null =
    await fetchEntries();
  ...
  const options = {
    renderMark: {
      ...
      [MARKS.BOLD]: (text: string) => (
        <strong className="text-bold">{text}</strong>
      ),
    },
    renderNode: {
      // Render <h3> elements with custom styling
      [BLOCKS.HEADING_3]: (node: any, children: any) => (
        <h3 className="text-2xl py-4">subTitle:{children}</h3>
      ),
      // Render paragraphs with custom styling
      [BLOCKS.PARAGRAPH]: (node: any, children: any) => (
        <p className="py-4">Paragraph:{children}</p>
      ),
    },
  };
        ...
        >
          ...
          <h2 className="text-2xl font-semibold py-4 text-white">
            <Link href={`/content/entries/${item.sys.id}`}>
              {typeof item.fields.articleTitle === "string"
